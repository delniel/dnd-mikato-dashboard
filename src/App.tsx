...
import { calculateSkillBonus, manaRecoveryAmount, thresholdForLevel, type CharacterState, type CurrencyKey, type Item, type Note, type ResourceKey, type Skill, type Spell } from './domain'
...
function Characteristics() {
  const state = useCharacterStore()
  return <div className="page-stack"><p className="page-intro">Проверки и спасброски задаются вручную. Навыки рассчитываются из числа характеристики; нажмите на навык, чтобы циклически выбрать владение ×0, ×1 или ×2. Поле «Бонус» рядом служит отдельной пометкой и в расчёт не входит.</p><section className="characteristic-grid">{state.characteristics.map((characteristic) => { const selected = state.profile.mainCharacteristic === characteristic.name; return <Card key={characteristic.id} className={selected ? 'characteristic is-main' : 'characteristic'}><div className="characteristic-heading"><h3>{characteristic.name}</h3><button type="button" className="main-stat" aria-label={`Сделать главной: ${characteristic.name}`} aria-pressed={selected} onClick={() => state.setProfile('mainCharacteristic', characteristic.name)}><Star size={17} fill={selected ? 'currentColor' : 'none'} /></button></div><div className="triple-fields">{(['score', 'check', 'save'] as const).map((field) => <FieldValue key={field} label={field === 'score' ? 'Число' : field === 'check' ? 'Проверка' : 'Спасбросок'} value={characteristic[field]} onChange={(value) => state.setCharacteristic(characteristic.id, field, value)} editable={state.editing} />)}</div><div className="skill-metrics">{characteristic.skills.map((skill) => { const value = calculateSkillBonus(characteristic.score, state.profile.proficiency, skill.proficiencyRank); const rankLabel = skill.proficiencyRank === 0 ? 'Без владения' : skill.proficiencyRank === 1 ? 'Владение ×1' : 'Владение ×2'; return <div key={skill.id} className={`skill-metric rank-${skill.proficiencyRank}`}><button type="button" className="skill-metric-main" aria-label={`${skill.name}: ${value === null ? 'не указано' : formatSignedCombatNumber(value)}. ${rankLabel}. Нажмите, чтобы изменить владение`} onClick={() => state.cycleSkillProficiency(characteristic.id, skill.id)}><span>{skill.name}</span><strong>{value === null ? '—' : formatSignedCombatNumber(value)}</strong><small>{rankLabel}</small></button><label className="skill-extra-bonus"><span>Бонус</span><input aria-label={skill.name} title={`Дополнительный бонус: ${skill.name}`} value={skill.bonus} placeholder="+0" onChange={(event) => state.setSkillDisplayBonus(characteristic.id, skill.id, event.target.value)} /></label></div> })}</div></Card> })}</section></div>
}
