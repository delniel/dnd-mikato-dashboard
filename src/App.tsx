...
import { calculateSkillBonus, characteristicModifier, manaRecoveryAmount, thresholdForLevel, type CharacterState, type CurrencyKey, type Item, type Note, type ResourceKey, type Skill, type Spell } from './domain'
            ...
            : page === 'Персонаж'
              ...
              : page === 'Заметки'
                ...
                : <SettingsPage persistCurrent={persistCurrent} applySingleImport={async (prepared, replace) => {
                    ...
                    materialized.character.data = applyLibrarySettings(materialized.character.data, current.settings)
                    if (materialized.images.length) await saveImages(materialized.images)
                    ...
                    for (const oldId of incoming.characterOrder) {
                      ...
                      materialized.character.data = applyLibrarySettings(materialized.character.data, next.settings)
                    ...
                    reset.data = applyLibrarySettings({ ...reset.data, settings: { ...reset.data.settings, accentColor: source.data.settings.accentColor } }, current.settings)
...
function CombatCalculationDetail({ calculation, onClose }: { calculation: CombatCalculation; onClose: () => void }) {
  ...
  return <Modal title={`Расчёт: ${calculation.target.label}`} onClose={onClose}>
    ...
    <p className="quiet calculation-order-hint">Порядок внутри категорий: установить, сложение/вычитание, затем для положительных и особых ÷ и ×, для отрицательных × и ÷. Категории: положительные → отрицательные → особые.</p>
    {calculation.setConflict && <p className="calculation-warning" role="alert">Несколько эффектов устанавливают значение через «=». Они применяются последовательно в порядке категорий и создания.</p>}
    {calculation.steps.length ? <ol className="calculation-steps">{calculation.steps.map((step, index) => <li key={`${step.effectId}-${index}`}><strong>{step.effectName}</strong><span>{combatOperationSymbols[step.operation]} {formatCombatNumber(step.value)}</span><span>{formatCombatNumber(step.before)} → {formatCombatNumber(step.after)}</span></li>)}</ol> : <p className="quiet">Активных числовых изменений для этого параметра нет.</p>}
  </Modal>
}
...
function Characteristics() {
  const state = useCharacterStore()
  return <div className="page-stack"><p className="page-intro">Проверки и спасброски задаются вручную. Нажмите на навык, чтобы добавить бонус владения один или два раза. Поле «Бонус» рядом служит отдельной пометкой и в расчёт не входит.</p><section className="characteristic-grid">{state.characteristics.map((characteristic) => { const selected = state.profile.mainCharacteristic === characteristic.name; const modifier = characteristicModifier(characteristic.score); return <Card key={characteristic.id} className={selected ? 'characteristic is-main' : 'characteristic'}><div className="characteristic-heading"><h3>{characteristic.name}{modifier !== null && <span className="characteristic-modifier"> ({formatSignedCombatNumber(modifier)})</span>}</h3><button type="button" className="main-stat" aria-label={`Сделать главной: ${characteristic.name}`} aria-pressed={selected} onClick={() => state.setProfile('mainCharacteristic', characteristic.name)}><Star size={17} fill={selected ? 'currentColor' : 'none'} /></button></div><div className="triple-fields">{(['score', 'check', 'save'] as const).map((field) => <FieldValue key={field} label={field === 'score' ? 'Число' : field === 'check' ? 'Проверка' : 'Спасбросок'} value={characteristic[field]} onChange={(value) => state.setCharacteristic(characteristic.id, field, value)} editable={state.editing} />)}</div><div className="skill-metrics">{characteristic.skills.map((skill) => { const value = calculateSkillBonus(state.profile.proficiency, skill.proficiencyRank); const rankLabel = skill.proficiencyRank === 0 ? 'Без владения' : skill.proficiencyRank === 1 ? 'Владение ×1' : 'Владение ×2'; return <div key={skill.id} className={`skill-metric rank-${skill.proficiencyRank}`}><button type="button" className="skill-metric-main" aria-label={`${skill.name}: ${formatSignedCombatNumber(value)}. ${rankLabel}. Нажмите, чтобы изменить владение`} onClick={() => state.cycleSkillProficiency(characteristic.id, skill.id)}><span>{skill.name}</span><strong>{formatSignedCombatNumber(value)}</strong><small>{rankLabel}</small></button><label className="skill-extra-bonus"><span>Бонус</span><input aria-label={skill.name} title={`Дополнительный бонус: ${skill.name}`} value={skill.bonus} placeholder="+0" onChange={(event) => state.setSkillDisplayBonus(characteristic.id, skill.id, event.target.value)} /></label></div> })}</div></Card> })}</section></div>
}
